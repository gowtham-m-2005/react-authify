...
const ResetPassword = () => {
    ...
    const handlePaste = (e, index) => {
        e.preventDefault()
        const clipBoard = e.clipboardData.getData("text").slice(0, 6).split("")
        clipBoard.forEach((digit, i) => {
            if (inputRef.current[i]) {
                inputRef.current[i].value = digit;
            }
        })
        const next = Math.min(clipBoard.length, 5)
    ...
    return (
        <div className="email-verify-container d-flex align-items-center justify-content-center vh-100 position-relative"
            ...
            <Link to="/" className="position-absolute top-0 start-0 p-4 d-flex align-items-center gap-2 text-decoration-none">
                <img src={assets.header} alt="logo" height={50} width={50} style={{ objectFit: "contain" }} />
                <span className="fs-4 fw-semibold text-light">Authify</span>
            </Link>

            {/* Enter Email Card */}
            {!isEmailSent && (
                <div className="rounded-4 p-5 bg-white text-center shadow" style={{ width: "100%", maxWidth: "400px" }}>
                    ...
                    <form onSubmit={onSubmitEmail}>
                        <div className="input-group mb-4 bg-secondary bg-opacity-10 rounded-pill">
                            ...
                            <input type="email"
                                   className="form-control bg-transparent border-0 ps-1 pe-4 rounded-end" placeholder="Enter your email" style={{ height: "50px" }}
            ...
            {isEmailSent && !isOtpSubmitted && (
                <div className="p-5 rounded-4 shadow bg-white" style={{ width: "400px" }}>
                    ...
                    <p className="text-center text-dark mb-4">Enter the OTP sent to your email</p>
                    <form>
                        ...
                        <button className="btn btn-primary w-100 fw-semibold py-2" onClick={handleVerify}>
                            Verify OTP
                        </button>
                    </form>
                </div>
            ...
            {isEmailSent && isOtpSubmitted && (
                <div className="rounded-4 p-5 bg-white text-center shadow" style={{ width: "100%", maxWidth: "400px" }}>
                    <h4 className="fw-bold mb-2">New Password</h4>
                    ...
                    <form onSubmit={onSubmitNewPassword}>
                        <div className="input-group mb-4 bg-secondary bg-opacity-10 rounded-pill">
                            ...
                            <input type="password"
                                   className="form-control bg-transparent border-0 ps-1 pe-4 rounded-end" placeholder="*****"
                                   style={{ height: "50px" }} onChange={(e) => setNewPassword(e.target.value)} value={newPassword} required />
                        </div>
